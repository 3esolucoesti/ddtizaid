...
class contactController extends controller{
	...
	public function index(){
		...
		if(isset($_POST['name']) && !empty($_POST['name'])){
			...
			if ($enviado) {
				$pg_contact->salvarmsg($name,$phone,$email,$assunto,$message);
	...
	public function newsletter() {
		...
		if(isset($_POST['newsletter-email']) && !empty($_POST['newsletter-email'])){
			$newsletter_email = addslashes($_POST['newsletter-email']);
			$return_url = $_POST['pg-url'];
			$pg_contact = new Pgcontact();

			if ($pg_contact->IsInNewsletter($newsletter_email))
				$msg = "Esse email ja foi cadastrado!";
			else 
				$pg_contact->addNewsLetter($newsletter_email);
				$msg = "Email cadastrado com sucesso!";
			
			echo '<script>alert("'.$msg.'")</script>';
			echo '<script>location.href=("'.BASE_URL.'/'.$return_url.'");</script>';
		}
	}
}
